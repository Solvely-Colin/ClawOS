...
if [[ -f "$installer" ]]; then
  ...
  grep -Fq 'noto-fonts inter-font otf-geist-mono-nerd' "$installer"
  grep -Fq 'otf-geist-mono-nerd' "$installer"
  ! grep -Eq 'otf-geist-mono-nerd[[:space:]]+\+' "$installer"
  grep -Fq 'networkmanager openssh sudo zsh tmux curl jq' "$installer"
  grep -Fq 'nodejs npm polkit lxqt-policykit plymouth tailscale' "$installer"
  ...
  grep -Fq 'gtklock gtklock-userinfo-module gtk-layer-shell python-gobject orca' "$installer"
