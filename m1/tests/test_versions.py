...
m1/config/versions.env is the single lock for the OpenClaw runtime that
...
VERSIONS_ENV = ROOT / "m1/config/versions.env"
CLAWOSD_CONFIG = ROOT / "m3/config/clawosd.json"
DELIVERY_ADAPTER = ROOT / "m2/openclaw-plugin/lib/deployment-delivery.js"
PLUGIN_MANIFEST = ROOT / "m2/openclaw-plugin/package.json"
...
MENTIONS = {
    ...
    "m2/SELF-DEVELOPMENT.md": dict(must_mention=True, allow_older=False),
    "m3/tests/test_clawosd.py": dict(must_mention=True, allow_older=False),
    "m2/openclaw-plugin/test/panel.test.js": dict(must_mention=True, allow_older=True),
    "m2/openclaw-plugin/package.json": dict(must_mention=False, allow_older=True),
}
