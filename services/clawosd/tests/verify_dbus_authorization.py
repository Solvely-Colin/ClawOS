...
core = load('clawosd_core', repo / 'services/clawosd/src/clawosd_core.py')
fixtures = load('broker_test_fixtures', repo / 'services/clawosd/tests/test_clawosd.py')
...
if len(sys.argv) > 2 and sys.argv[2] == 'serve':
    ...
    adapter = load('broker_adapter', repo / 'services/clawosd/src/clawosd_service.py')
...
with tempfile.TemporaryDirectory(prefix='clawos-bus-proof-') as tmp:
    root = Path(tmp)
    root.chmod(0o755)
    config = json.loads((repo/'services/clawosd/config/clawosd.json').read_text())
