...
export function deployRuntimeSessionHint(options = {}) {
  ...
  return `\nFor runtime apply, bind automatic completion delivery with ./image/bin/deploy-runtime apply --session-key ${sessionKey}. Dispatch once, then end with the job ID and pending state. ClawOS delivers the verified terminal receipt here automatically; do not hold the old turn open polling.`;
}
