...
| --- | --- | --- |
| Supported in VM | Live ISO boots under UEFI/OVMF; installed system boots without the ISO through its systemd-boot entry; LUKS unlock with the typed passphrase | WHPX: [HARDWARE-INSTALLER-VALIDATION.md](HARDWARE-INSTALLER-VALIDATION.md) "2026-09-07: sshd policy and both install modes re-verified". The ISO workflow now requires `image/tests/boot-smoke-qemu`; the stronger install gate `image/tests/m2-e2e-qemu` remains separate. ISO structure: `image/tests/validate-iso.sh`, run by `build-iso` |
...
| Experimental | CI-built ISO (`release.yml`); physical x86_64 UEFI hardware | Run 34432248984 at `de19c1b` was manually installed and booted under WHPX on 2026-09-10. Current workflow requires structural validation and live KVM boot, not installation. No physical machine installed; manual ISO retention is opt-in ([EVIDENCE.md](EVIDENCE.md)) |
