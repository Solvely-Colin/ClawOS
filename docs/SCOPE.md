...
| Experimental | CI-built ISO (`release.yml`); physical x86_64 UEFI hardware | Workflow requires structural validation, live KVM boot, boot/signature/mounted-target refusal proof and passwordless install/disk-only/updater-readiness checks. Run 34522981039 also passed the fresh WHPX core agent loop without runtime repair. No physical machine installed; manual ISO retention is opt-in and temporary ISO artifacts are not retained by default ([EVIDENCE.md](EVIDENCE.md)) |
