...
| --- | --- | --- |
| Image assembly | `m1/bin/materialize-profile`, `m1/bin/build-iso` | Combine the ArchISO overlay, plugin and broker into a Linux-built image |
| Installed desktop and setup | `m1/profile-overlay/airootfs` | Sway session, panel, installer, onboarding, application registry and supporting surfaces |
| OpenClaw integration | `m2/openclaw-plugin` | Typed tools and minimal activity projection; no parallel conversation store |
| System broker | `m3/clawosd`, `m3/dbus`, `m3/polkit` | Machine actions, approval boundaries and receipts; see SECURITY.md |
| Role integration | `clawos-role` in the overlay; `m4/tests` | Standalone/remote-node switching and fixture-based regression checks |
...
[live development](../m2/SELF-DEVELOPMENT.md).

`m0` is a historical executable fixture still checked by preflight.
