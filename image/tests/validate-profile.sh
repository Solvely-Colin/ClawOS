...
profile="${1:-$repo_root/image/profile-overlay}"
...
"$repo_root/image/tests/check-distribution-boundary.sh"

grep -Fqx "Server = https://archive.archlinux.org/repos/${ARCH_SNAPSHOT}/\$repo/os/\$arch" \
  "$repo_root/image/config/mirrorlist"
...
for package in base chromium foot fuzzel gtk-layer-shell gtk3 inter-font linux linux-firmware lxqt-policykit mesa mkinitcpio-archiso networkmanager nodejs noto-fonts npm openssh orca plymouth python-gobject sway swaybg syslinux waybar zsh; do
  grep -Fqx "$package" "$repo_root/image/profile-overlay/packages.x86_64"
done
...
if [[ -f "$archiso_mkinitcpio" ]] && grep -Fq 'memdisk' "$archiso_mkinitcpio"; then
  grep -Fqx 'syslinux' "$repo_root/image/profile-overlay/packages.x86_64"
fi

if [[ -f "$archiso_mkinitcpio" ]] && grep -Fq 'archiso' "$archiso_mkinitcpio"; then
  grep -Fqx 'mkinitcpio-archiso' "$repo_root/image/profile-overlay/packages.x86_64"
fi

if [[ -f "$profile/airootfs/etc/passwd" ]] && \
   grep -Eq '^root:.*:/usr/bin/zsh$' "$profile/airootfs/etc/passwd"; then
  grep -Fqx 'zsh' "$repo_root/image/profile-overlay/packages.x86_64"
fi

grep -Fq "bootmodes=('uefi.systemd-boot')" "$repo_root/image/profile-overlay/profiledef.sh"
if grep -Fq 'bios.' "$repo_root/image/profile-overlay/profiledef.sh"; then
...
grep -Fq -- '--headless-vnc' "$repo_root/image/bin/run-qemu"
grep -Fq -- '-vnc "127.0.0.1:$vnc_display"' "$repo_root/image/bin/run-qemu"
if grep -Fq -- '-vnc "0.0.0.0' "$repo_root/image/bin/run-qemu"; then
...
if [[ "$profile" == "$repo_root/image/profile-overlay" ]]; then
  "$repo_root/image/tests/onboarding-static.sh"
fi
